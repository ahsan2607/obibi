\documentclass[10pt, aspectratio=169]{beamer}

% === TEMA ===
\usetheme{Madrid}
\usecolortheme{default}

% === PAKET TAMBAHAN ===
\usepackage[utf8]{inputenc}
\usepackage{graphicx}
\usepackage{hyperref}
\usepackage{booktabs}
\usepackage{multirow}
\usepackage{array}
\usepackage{xcolor}
\usepackage{listings}
\usepackage{tikz}
\usetikzlibrary{arrows.meta, positioning, shapes.geometric, fit}

% === PENGATURAN WARNA ===
\definecolor{mainblue}{RGB}{0, 102, 204}
\definecolor{lightblue}{RGB}{230, 242, 255}
\definecolor{darkblue}{RGB}{0, 51, 102}

\setbeamercolor{titlelike}{bg=mainblue, fg=white}
\setbeamercolor{frametitle}{bg=mainblue, fg=white}
\setbeamercolor{structure}{fg=mainblue}
\setbeamercolor{block title}{bg=mainblue, fg=white}
\setbeamercolor{block body}{bg=lightblue, fg=black}

% === INFORMASI DOKUMEN ===
\title{Aplikasi Obibi: Pengingat Jadwal Obat Pintar}
\subtitle{Presentasi SKPL, DPPL, \& DUPL}
\author{Najla Tsabita Afiyah \and Revalina Rasyidin \and Ravi Adi Prakoso \and Muhammad Ahsan Raziq Sulthany}
\institute[Universitas Telkom]{\small Program Studi S1 Informatika\\Fakultas Informatika\\Universitas Telkom}
\date{2026}

% === AWAL DOKUMEN ===
\begin{document}

% --- SLIDE JUDUL ---
\begin{frame}
    \titlepage
    \begin{center}
        \tiny{Dosen Pengampu: Helmi Faisal Muttaqin, S.Kom., M.T.}
    \end{center}
\end{frame}

% --- DAFTAR ISI ---
\begin{frame}{Daftar Isi}
    \tableofcontents
\end{frame}

% ======================================================================
% BAGIAN 1: PENDAHULUAN & GAMBARAN UMUM
% ======================================================================
\section{Pendahuluan \& Gambaran Umum}

% --- SLIDE: LATAR BELAKANG ---
\begin{frame}{Latar Belakang}
    \begin{itemize}
        \item Banyak pasien kronis, lansia, dan penyandang disabilitas kesulitan mengikuti jadwal pengobatan.
        \item Keterbatasan fisik, kognitif, dan akses teknologi menjadi kendala utama.
        \item Dibutuhkan solusi digital yang \textbf{inklusif, mudah digunakan, dan cerdas}.
    \end{itemize}
    \vspace{0.5cm}
    \begin{block}{Solusi: Aplikasi Obibi}
        Asisten kesehatan berbasis \textbf{Agentic AI} dan \textbf{GraphRAG} untuk:
        \begin{itemize}
            \item Pengingat obat otomatis dan personal.
            \item Analisis interaksi antar obat secara mandiri.
            \item Chatbot AI dengan dukungan perintah suara.
        \end{itemize}
    \end{block}
\end{frame}

% --- SLIDE: TUJUAN PRODUK ---
\begin{frame}{Tujuan \& Manfaat Produk}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \textbf{Tujuan Utama:}
        \begin{itemize}
            \item Meningkatkan kepatuhan pasien terhadap pengobatan.
            \item Menyediakan pengingat yang terpersonalisasi dan inklusif.
            \item Memberikan akses informasi kesehatan yang aman dan akurat.
        \end{itemize}
        
        \column{0.5\textwidth}
        \textbf{Manfaat:}
        \begin{itemize}
            \item \textbf{Pasien}: Pengingat otomatis, asisten virtual 24/7.
            \item \textbf{Tenaga Medis}: Membantu memantau kepatuhan pasien.
            \item \textbf{Ekosistem Kesehatan}: Model sistem kesehatan inklusif berbasis AI.
        \end{itemize}
    \end{columns}
\end{frame}

% --- SLIDE: PROFIL PENGGUNA ---
\begin{frame}{Profil \& Kelas Pengguna}
    \begin{itemize}
        \item \textbf{Pasien (Utama)}:
        \begin{itemize}
            \item Lansia dan penyandang disabilitas.
            \item Pasien kronis dengan pengobatan rutin.
            \item Kemampuan teknis rendah hingga menengah.
        \end{itemize}
        \item \textbf{Medical Admin / Apoteker (Sekunder)}:
        \begin{itemize}
            \item Mengelola \textit{Knowledge Base} obat dan interaksi.
            \item Memutakhirkan data medis berdasarkan literatur terbaru.
        \end{itemize}
    \end{itemize}
\end{frame}

% ======================================================================
% BAGIAN 2: KEBUTUHAN PERANGKAT LUNAK (SKPL)
% ======================================================================
\section{Spesifikasi Kebutuhan Perangkat Lunak}

% --- SLIDE: KEBUTUHAN FUNGSIONAL (FR) ---
\begin{frame}{Kebutuhan Fungsional (FR)}
    \begin{table}[]
    \tiny
    \begin{tabular}{|c|p{0.45\textwidth}|c|}
        \hline
        \textbf{No} & \textbf{Nama Fungsional} & \textbf{Tipe} \\
        \hline
        FR01 & Manajemen Akun Pengguna (Login/Register) & Must Have \\
        FR02 & Manajemen Data Obat & Must Have \\
        FR03 & Manajemen Jadwal Konsumsi Obat & Must Have \\
        FR04 & Notifikasi Pengingat Obat & Must Have \\
        FR05 & Catat Konsumsi Obat (Mark as Taken) & Must Have \\
        FR06 & Laporan Kepatuhan & Satisfy \\
        FR07 & Deteksi \& Manajemen Interaksi Obat & Satisfy \\
        FR08 & Chat dengan Asisten AI (Obibi) & Delighter \\
        FR09 & Tambah Obat Otomatis lewat AI & Delighter \\
        FR10 & Konversi Suara (STT \& TTS) & Satisfy \\
        \hline
    \end{tabular}
    \end{table}
    \vspace{0.2cm}
    \begin{flushright}
        \tiny{\textit{Sumber: SKPL Aplikasi Obibi}}
    \end{flushright}
\end{frame}

% --- SLIDE: KEBUTUHAN NON-FUNGSIONAL (NFR) ---
\begin{frame}{Kebutuhan Non-Fungsional (NFR)}
    \begin{table}[]
    \tiny
    \begin{tabular}{|c|p{0.4\textwidth}|c|}
        \hline
        \textbf{No} & \textbf{Kriteria Kualitas} & \textbf{Kode} \\
        \hline
        1 & \textbf{Usability}: SUS minimal 80, ramah lansia/disabilitas & NFR-01 \\
        2 & \textbf{Reliability}: Uptime 99\%, pengingat tetap jalan meski offline & NFR-02 \\
        3 & \textbf{Performance}: Loading < 3 detik, AI < 10 detik & NFR-03 \\
        4 & \textbf{Security}: Enkripsi AES-256, OAuth 2.0 & NFR-04 \\
        5 & \textbf{Availability}: 24/7, fallback mechanism & NFR-05 \\
        6 & \textbf{Maintainability}: Arsitektur modular & NFR-06 \\
        7 & \textbf{Scalability}: 10.000 pengguna aktif harian & NFR-07 \\
        8 & \textbf{Portability}: Android ≥ v8.0, iOS ≥ v12 & NFR-08 \\
        9 & \textbf{Accessibility}: Voice interaction, screen reader & NFR-09 \\
        10 & \textbf{Compliance}: UU PDP, ISO 27001 & NFR-10 \\
        \hline
    \end{tabular}
    \end{table}
\end{frame}

% ======================================================================
% BAGIAN 3: PERANCANGAN PERANGKAT LUNAK (DPPL)
% ======================================================================
\section{Perancangan Perangkat Lunak}

% --- SLIDE: ARSITEKTUR SISTEM ---
\begin{frame}{Arsitektur Perangkat Lunak}
    \begin{center}
        \begin{tikzpicture}[node distance=1.2cm, auto, >=Stealth]
            % Nodes
            \node (client) [rectangle, draw, fill=lightblue, minimum width=2cm, minimum height=0.8cm] {Client (Browser)};
            \node (server) [rectangle, draw, fill=lightblue, minimum width=2cm, minimum height=0.8cm, below=of client] {Server (Next.js)};
            \node (ai) [rectangle, draw, fill=lightblue, minimum width=2cm, minimum height=0.8cm, below=of server] {AI Engine (LLM \& GraphRAG)};
            \node (db) [rectangle, draw, fill=lightblue, minimum width=2cm, minimum height=0.8cm, below=of ai] {Database (PostgreSQL)};
            
            % Arrows
            \draw[->] (client) -- node[right] {HTTPS} (server);
            \draw[->] (server) -- node[right] {API} (ai);
            \draw[->] (server) -- node[right] {Query} (db);
            \draw[<->] (ai) -- node[right] {Data} (db);
            
            % Notes
            \node [above right=0.2cm and -0.5cm of client] {\tiny{React + Supabase}};
            \node [above right=0.2cm and -0.5cm of server] {\tiny{API Routes + Middleware}};
            \node [above right=0.2cm and -0.5cm of ai] {\tiny{Agentic AI + GraphRAG}};
            \node [above right=0.2cm and -0.5cm of db] {\tiny{Supabase/PostgreSQL}};
        \end{tikzpicture}
    \end{center}
    \vspace{0.2cm}
    \begin{flushright}
        \tiny{\textit{Sumber: DPPL Aplikasi Obibi}}
    \end{flushright}
\end{frame}

% --- SLIDE: USE CASE DIAGRAM ---
\begin{frame}{Use Case Diagram}
    \begin{center}
        \begin{tikzpicture}[scale=0.75, every node/.style={transform shape}]
          % Actors
          \draw[fill=lightblue] (0, 2) circle (0.3);
          \draw (0, 1.7) -- (0, 0.7);
          \draw (-0.5, 1.4) -- (0.5, 1.4);
          \draw (0, 0.7) -- (-0.3, 0);
          \draw (0, 0.7) -- (0.3, 0);
          \node at (0, -0.3) {\small Pasien};

          \draw[fill=lightblue] (10, 2) circle (0.3);
          \draw (10, 1.7) -- (10, 0.7);
          \draw (9.5, 1.4) -- (10.5, 1.4);
          \draw (10, 0.7) -- (9.7, 0);
          \draw (10, 0.7) -- (10.3, 0);
          \node at (10, -0.3) {\small Admin};

          % System Boundary
          \draw (2.5, -3) rectangle (7.5, 5);
          \node at (5, 4.7) {\textbf{Aplikasi Obibi}};

          % Use Cases
          \node[draw, ellipse, fill=white, minimum width=2.5cm] (uc1) at (5, 3.8) {\tiny Manajemen Akun};
          \node[draw, ellipse, fill=white, minimum width=2.5cm] (uc2) at (5, 2.7) {\tiny Kelola Data Obat};
          \node[draw, ellipse, fill=white, minimum width=2.5cm] (uc3) at (5, 1.6) {\tiny Kelola Jadwal Obat};
          \node[draw, ellipse, fill=white, minimum width=2.5cm] (uc4) at (5, 0.5) {\tiny Cek Interaksi Obat};
          \node[draw, ellipse, fill=white, minimum width=2.5cm] (uc5) at (5, -0.6) {\tiny Chat Asisten AI};
          \node[draw, ellipse, fill=white, minimum width=2.5cm] (uc6) at (5, -1.7) {\tiny Laporan Kepatuhan};

          % Associations
          \draw (0.3, 1.4) -- (uc1.west);
          \draw (0.3, 1.4) -- (uc2.west);
          \draw (0.3, 1.4) -- (uc3.west);
          \draw (0.3, 1.4) -- (uc4.west);
          \draw (0.3, 1.4) -- (uc5.west);
          \draw (0.3, 1.4) -- (uc6.west);

          \draw (9.7, 1.4) -- (uc1.east);
          \draw (9.7, 1.4) -- (uc2.east);
        \end{tikzpicture}
    \end{center}
    \begin{flushright}
        \tiny{\textit{Sumber: SKPL \& DPPL Aplikasi Obibi}}
    \end{flushright}
\end{frame}

% --- SLIDE: ERD ---
\begin{frame}{Entity Relationship Diagram (ERD)}
    \begin{center}
        \includegraphics[width=0.65\textwidth]{db.jpeg}
    \end{center}
    \begin{flushright}
        \tiny{\textit{Sumber: SKPL \& DPPL Aplikasi Obibi}}
    \end{flushright}
\end{frame}

% --- SLIDE: DIAGRAM KELAS ---
\begin{frame}{Diagram Kelas (Perancangan)}
    \begin{center}
        \includegraphics[width=0.55\textwidth]{class_diagram.jpeg}
    \end{center}
    \begin{flushright}
        \tiny{\textit{Sumber: DPPL Aplikasi Obibi}}
    \end{flushright}
\end{frame}

% --- SLIDE: USE CASE SCENARIO #1 ---
\begin{frame}{Use Case: Registrasi Pengguna Baru}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \textbf{Skenario Normal:}
        \begin{enumerate}
            \item Calon pengguna membuka halaman register.
            \item Mengisi data diri (Nama, Email, Kata Sandi).
            \item Menekan tombol "Register".
            \item Sistem memvalidasi data dan menyimpan ke database.
            \item Pengguna diarahkan ke halaman login.
        \end{enumerate}
        
        \column{0.5\textwidth}
        \textbf{Alternatif:}
        \begin{itemize}
            \item Email sudah terdaftar: Sistem menampilkan peringatan.
            \item Koneksi internet terputus: Sistem menampilkan pesan error koneksi.
        \end{itemize}
        \vspace{0.3cm}
        \textbf{Post-Condition:}
        \begin{itemize}
            \item Akun pengguna berhasil terdaftar di database.
        \end{itemize}
    \end{columns}
\end{frame}

% --- SLIDE: USE CASE SCENARIO #2 ---
\begin{frame}{Use Case: Login Pengguna}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \textbf{Skenario Normal:}
        \begin{enumerate}
            \item Pengguna membuka halaman login.
            \item Memasukkan Email dan Kata Sandi yang valid.
            \item Menekan tombol "Login".
            \item Sistem memvalidasi kredensial pengguna.
            \item Pengguna diarahkan ke dashboard utama.
        \end{enumerate}
        
        \column{0.5\textwidth}
        \textbf{Alternatif:}
        \begin{itemize}
            \item Kredensial tidak valid: Sistem menampilkan pesan kesalahan login.
            \item Koneksi terputus: Tampil pesan kesalahan jaringan.
        \end{itemize}
        \vspace{0.3cm}
        \textbf{Post-Condition:}
        \begin{itemize}
            \item Sesi pengguna aktif dibuat dan masuk ke dashboard.
        \end{itemize}
    \end{columns}
\end{frame}

% --- SLIDE: USE CASE SCENARIO #3 ---
\begin{frame}{Use Case: Kelola Data Obat}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \textbf{Skenario Normal:}
        \begin{enumerate}
            \item Pasien membuka menu Kelola Obat.
            \item Klik Tambah Obat.
            \item Isi form (Nama, Deskripsi, Dosis, Stok, Bentuk).
            \item Klik Simpan Obat.
            \item Sistem validasi data dan simpan ke database.
        \end{enumerate}
        
        \column{0.5\textwidth}
        \textbf{Alternatif:}
        \begin{itemize}
            \item Input kosong / tidak valid: sistem menampilkan pesan kesalahan.
            \item Hapus obat: Pasien mengonfirmasi dialog hapus, sistem menghapus obat beserta jadwal terkait secara cascade.
        \end{itemize}
        \vspace{0.3cm}
        \textbf{Post-Condition:}
        \begin{itemize}
            \item Data obat terupdate di database dan muncul di daftar.
        \end{itemize}
    \end{columns}
\end{frame}

% --- SLIDE: USE CASE SCENARIO #4 ---
\begin{frame}{Use Case: Input Jadwal Minum Obat}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \textbf{Skenario Normal:}
        \begin{enumerate}
            \item Pasien buka halaman Kelola Obat.
            \item Klik Tambah Jadwal.
            \item Isi form (Tipe, Waktu, Tanggal).
            \item Klik Simpan Jadwal.
            \item Sistem validasi dan simpan ke database.
            \item \textit{Cronjob} reminder aktif.
        \end{enumerate}
        
        \column{0.5\textwidth}
        \textbf{Alternatif:}
        \begin{itemize}
            \item Validasi gagal: tampilkan pesan error.
            \item Database error: notifikasi gagal simpan.
            \item Cronjob gagal: log error, retry otomatis.
        \end{itemize}
        \vspace{0.3cm}
        \textbf{Post-Condition:}
        \begin{itemize}
            \item Jadwal tersimpan, cronjob aktif.
        \end{itemize}
    \end{columns}
\end{frame}

% --- SLIDE: USE CASE SCENARIO #5 ---
\begin{frame}{Use Case: Cek Interaksi Antar Obat}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \textbf{Skenario Normal:}
        \begin{enumerate}
            \item Pasien membuka menu interaksi.
            \item Pasien mengecek interaksi antar obat.
            \item Sistem AI mengecek data jurnal klinis (GraphRAG).
            \item Sistem mencari konteks dan membuat kesimpulan.
            \item Pasien menerima hasil analisis \& rekomendasi.
        \end{enumerate}
        
        \column{0.5\textwidth}
        \textbf{Alternatif:}
        \begin{itemize}
            \item Jika obat tidak ditemukan, sistem menyarankan konsultasi dokter.
        \end{itemize}
        \vspace{0.3cm}
        \textbf{Post-Condition:}
        \begin{itemize}
            \item Pasien melihat kesimpulan interaksi obat.
        \end{itemize}
    \end{columns}
\end{frame}

% --- SLIDE: USE CASE SCENARIO #6 ---
\begin{frame}{Use Case: Chat dengan Asisten AI (Obibi)}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \textbf{Skenario Normal:}
        \begin{enumerate}
            \item Pasien membuka halaman Chat.
            \item Mengetik pertanyaan medis / merekam perintah suara.
            \item Menekan kirim.
            \item AI Engine memproses pertanyaan dengan GraphRAG.
            \item Sistem menampilkan respon asisten virtual kesehatan.
        \end{enumerate}
        
        \column{0.5\textwidth}
        \textbf{Alternatif:}
        \begin{itemize}
            \item AI Engine lambat / timeout: Menampilkan pesan fallback untuk mencoba lagi.
            \item Suara tidak jelas (STT): Meminta pengguna mengulangi perintah suara.
        \end{itemize}
        \vspace{0.3cm}
        \textbf{Post-Condition:}
        \begin{itemize}
            \item Riwayat percakapan pasien tersimpan.
        \end{itemize}
    \end{columns}
\end{frame}

% --- SLIDE: SEQUENCE DIAGRAM ---
\begin{frame}{Sequence Diagram: Input Jadwal Obat}
    \begin{center}
        \begin{tikzpicture}[scale=0.75, every node/.style={transform shape}]
          % Lifelines
          \draw[thick, dashed] (0, 4) -- (0, 0);
          \draw[thick, dashed] (3, 4) -- (3, 0);
          \draw[thick, dashed] (6, 4) -- (6, 0);
          \draw[thick, dashed] (9, 4) -- (9, 0);

          % Objects
          \node[draw, fill=lightblue, minimum width=1.5cm] at (0, 4.3) {\small Pasien};
          \node[draw, fill=lightblue, minimum width=1.5cm] at (3, 4.3) {\small UI Halaman};
          \node[draw, fill=lightblue, minimum width=1.5cm] at (6, 4.3) {\small Server API};
          \node[draw, fill=lightblue, minimum width=1.5cm] at (9, 4.3) {\small Database};

          % Messages
          \draw[->, >=Stealth] (0, 3.5) -- node[above] {\tiny 1. Isi Form Jadwal} (3, 3.5);
          \draw[->, >=Stealth] (3, 3.0) -- node[above] {\tiny 2. Kirim Data Jadwal} (6, 3.0);
          \draw[->, >=Stealth] (6, 2.5) -- node[above] {\tiny 3. Simpan (Query)} (9, 2.5);
          
          \draw[->, >=Stealth, dashed] (9, 1.8) -- node[above] {\tiny 4. Konfirmasi Sukses} (6, 1.8);
          \draw[->, >=Stealth, dashed] (6, 1.2) -- node[above] {\tiny 5. Respons Sukses (JSON)} (3, 1.2);
          \draw[->, >=Stealth, dashed] (3, 0.6) -- node[above] {\tiny 6. Tampilkan Notifikasi} (0, 0.6);
        \end{tikzpicture}
    \end{center}
    \begin{flushright}
        \tiny{\textit{Sumber: DPPL Aplikasi Obibi}}
    \end{flushright}
\end{frame}

% ======================================================================
% BAGIAN 4: PENGUJIAN PERANGKAT LUNAK (DUPL)
% ======================================================================
\section{Pengujian Perangkat Lunak}

% --- SLIDE: STRATEGI PENGUJIAN ---
\begin{frame}{Strategi \& Metode Pengujian}
    \begin{itemize}
        \item \textbf{Black-Box Testing}: Menguji fungsionalitas tanpa melihat kode internal.
        \item \textbf{Boundary Value Analysis (BVA)}: Validasi input ekstrem (format waktu, input kosong).
        \item \textbf{User Acceptance Testing (UAT)}: Melibatkan \textit{end-user} eksternal untuk memastikan kemudahan penggunaan.
    \end{itemize}
    \vspace{0.5cm}
    \begin{table}[]
    \small
    \begin{tabular}{|c|c|c|}
        \hline
        \textbf{Use Case} & \textbf{PIC} & \textbf{Jadwal} \\
        \hline
        Login & Ravi Adi Prakoso & 28 Mei 2026 \\
        Register & Muhammad Ahsan Raziq & 28 Mei 2026 \\
        Chatbot & Najla Tsabita & 28 Mei 2026 \\
        Input Obat & Revalina Rasyidin & 28 Mei 2026 \\
        Input Jadwal Obat & Ravi Adi Prakoso & 28 Mei 2026 \\
        \hline
    \end{tabular}
    \end{table}
\end{frame}

% --- SLIDE: HASIL PENGUJIAN ---
\begin{frame}{Hasil Pengujian Fungsional}
    \begin{table}[]
    \tiny
    \begin{tabular}{|c|c|c|c|}
        \hline
        \textbf{ID DUPL} & \textbf{Use Case} & \textbf{Skenario} & \textbf{Status} \\
        \hline
        DUPL-LOGIN-01 & Login & Login Berhasil & ✓ Valid \\
        DUPL-LOGIN-02 & Login & Login Gagal karena Internet & ✓ Valid \\
        DUPL-REGISTER-01 & Register & Register Berhasil & ✓ Valid \\
        DUPL-REGISTER-02 & Register & Register Gagal karena Internet & ✓ Valid \\
        DUPL-OBAT-01 & Kelola Obat & Input Data Obat Berhasil & ✓ Valid \\
        DUPL-OBAT-02 & Kelola Obat & Validasi Nama Obat Kosong & ✓ Valid \\
        DUPL-OBAT-03 & Kelola Obat & Hapus Data Obat Berhasil & ✓ Valid \\
        DUPL-INTERAKSI-01 & Cek Interaksi & Interaksi Obat Valid & ✓ Valid \\
        DUPL-INTERAKSI-02 & Cek Interaksi & Obat Tidak Ditemukan & ✓ Valid \\
        DUPL-INTERAKSI-03 & Cek Interaksi & Input Kosong & ✓ Valid \\
        DUPL-JADWAL-01 & Input Jadwal & Tambah Jadwal Berhasil & ✓ Valid \\
        DUPL-JADWAL-02 & Input Jadwal & Validasi Format Waktu & ✓ Valid \\
        DUPL-JADWAL-03 & Input Jadwal & Database Gagal Simpan & ✓ Valid \\
        \hline
    \end{tabular}
    \end{table}
    \vspace{0.3cm}
    \begin{block}{Kesimpulan}
        Semua butir uji dinyatakan \textbf{lolos} (\textit{All Clear}).
    \end{block}
\end{frame}

% --- SLIDE: MATRIKS PENGUJIAN TERHADAP REQUIREMENT ---
\begin{frame}{Matriks Pengujian terhadap FR}
    \begin{table}[]
    \begin{tabular}{|c|c|c|}
        \hline
        \textbf{Kode FR} & \textbf{Functional Requirement} & \textbf{ID DUPL} \\
        \hline
        FR-01 & Manajemen Akun Pengguna & DUPL-LOGIN-01, DUPL-LOGIN-02, DUPL-REGISTER-01, DUPL-REGISTER-02 \\
        FR-03 & Kelola Data Obat & DUPL-OBAT-01 s.d. 03 \\
        FR-05 & Analisis Interaksi Obat & DUPL-INTERAKSI-01 s.d. 03 \\
        FR-07 & Personalisasi Jadwal Pengobatan & DUPL-JADWAL-01 s.d. 03 \\
        \hline
    \end{tabular}
    \end{table}
    \vspace{0.5cm}
    \begin{flushright}
        \tiny{\textit{Sumber: DUPL Aplikasi Obibi}}
    \end{flushright}
\end{frame}

% ======================================================================
% BAGIAN 5: KESIMPULAN
% ======================================================================
\section{Kesimpulan}

% --- SLIDE: KESIMPULAN ---
\begin{frame}{Kesimpulan}
    \begin{itemize}
        \item Aplikasi Obibi telah berhasil dirancang dan diuji sesuai dengan kebutuhan fungsional dan non-fungsional.
        \item Fitur utama (Login, Register, Cek Interaksi Obat, Input Jadwal) berfungsi dengan baik dan lolos pengujian.
        \item Sistem dibangun dengan arsitektur yang modular dan skalabel, siap untuk dikembangkan lebih lanjut.
        \item Pengujian \textit{User Acceptance} menunjukkan bahwa aplikasi ini \textbf{ramah pengguna} dan sesuai untuk target audiens (lansia dan penyandang disabilitas).
    \end{itemize}
    \vspace{0.5cm}
    \begin{block}{Rekomendasi}
        Pengembangan lebih lanjut pada fitur \textit{delighter} (AI Chatbot, Voice Command) dan integrasi dengan sistem eksternal.
    \end{block}
\end{frame}

% --- SLIDE: TERIMA KASIH ---
\begin{frame}
    \begin{center}
        \Huge{\textbf{TERIMA KASIH}}
        
        \vspace{0.5cm}
        \Large{Ada pertanyaan?}
        
        \vspace{1cm}
        \small{Dipersembahkan oleh:}
        
        \normalsize{Najla Tsabita Afiyah (103012400305)}
        
        \normalsize{Revalina Rasyidin (103012400156)}
        
        \normalsize{Ravi Adi Prakoso (103012430058)}
        
        \normalsize{Muhammad Ahsan Raziq Sulthany (103012430067)}
    \end{center}
\end{frame}

% ======================================================================
% LAMPIRAN: SCREENSHOT HASIL PENGUJIAN
% ======================================================================
\appendix
\section{Lampiran}

% --- SLIDE: SCREENSHOT PENGUJIAN ---
\begin{frame}{Lampiran: Screenshot Pengujian}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \textbf{Login Sukses}
        \begin{center}
            \includegraphics[width=0.8\textwidth]{loginberhasil.png}
        \end{center}
        
        \column{0.5\textwidth}
        \textbf{Register Sukses}
        \begin{center}
            \includegraphics[width=0.8\textwidth]{registerberhasil.png}
        \end{center}
    \end{columns}
    \begin{flushright}
        \tiny{\textit{Sumber: DUPL Aplikasi Obibi}}
    \end{flushright}
\end{frame}

% --- SLIDE: SCREENSHOT PENGUJIAN (2) ---
\begin{frame}{Lampiran: Screenshot Pengujian (2)}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \textbf{Error Koneksi}
        \begin{center}
            \includegraphics[width=0.8\textwidth]{regErrornointernet.png}
        \end{center}
        
        \column{0.5\textwidth}
        \textbf{Hasil Interaksi Obat}
        \begin{center}
            \includegraphics[width=0.8\textwidth]{laporankepatuhan.png}
        \end{center}
    \end{columns}
\end{frame}

% --- SLIDE: SCREENSHOT PENGUJIAN (3) ---
\begin{frame}{Lampiran: Screenshot Pengujian (3)}
    \begin{center}
        \textbf{Input Jadwal Obat}
        \includegraphics[width=0.6\textwidth]{jadwal_obat.jpeg}
    \end{center}
    \begin{flushright}
        \tiny{\textit{Sumber: DUPL Aplikasi Obibi}}
    \end{flushright}
\end{frame}

% --- SLIDE: SCREENSHOT PENGUJIAN (4) ---
\begin{frame}{Lampiran: Screenshot Pengujian (4)}
    \begin{center}
        \textbf{Kelola Data Obat}
        \includegraphics[width=0.6\textwidth]{kelola_obat.png}
    \end{center}
    \begin{flushright}
        \tiny{\textit{Sumber: DUPL Aplikasi Obibi}}
    \end{flushright}
\end{frame}

\end{document}